...
public class MastermindDatabaseService extends ADatabaseService {
    ...
    public void insertScore(String userId, LocalDate date, int attempts, long timeSeconds) {
        String sql = "INSERT OR IGNORE INTO mastermind_scores (user_id, date, attempts, time_seconds, completed_at) VALUES (?, ?, ?, ?, ?)";
        dbManager.executeUpdate(sql, pstmt -> {
            try {
                ...
                logger.error("Failed to insert mastermind score", e);
            }
        });
    }

    public boolean hasCompletedToday(String userId, LocalDate date) {
        String sql = "SELECT COUNT(*) FROM mastermind_scores WHERE user_id = ? AND date = ?";
        return dbManager.executeQueryForInt(sql, pstmt -> {
            try {
                pstmt.setString(1, userId);
                pstmt.setString(2, date.toString());
            } catch (SQLException e) {
                logger.error("Failed to check mastermind completion", e);
            }
        }) > 0;
    }

    public List<ScoreEntry> getDailyScores(LocalDate date) {
        String sql = "SELECT user_id, attempts, time_seconds FROM mastermind_scores WHERE date = ? ORDER BY attempts ASC, time_seconds ASC";
        return dbManager.executeQuery(sql, pstmt -> {
            try {
                pstmt.setString(1, date.toString());
            } catch (SQLException e) {
                logger.error("Failed to get daily mastermind scores", e);
            }
        }, rs -> {
            try {
                ...
                logger.error("Failed to read mastermind score", e);
                return null;
            }
        });
    }
    ...
    public List<AllTimeEntry> getAllTimeScores() {
        ...
        return dbManager.executeQuery(sql, pstmt -> {}, rs -> {
            try {
                ...
                logger.error("Failed to read all-time mastermind score", e);
                return null;
            }
        });
    }
